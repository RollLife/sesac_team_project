...
| 등급 갱신 | 7일마다 6개월 누적 기준 (느슨한 강등) |
| 장바구니 | 1회 쇼핑 = 1~10개 상품 구매 |
| 가격 분포 | 베타분포(2,5) 저가 치우침 |
...
| 등급 갱신 | `grade_updater.py`의 6개월 누적 기준 (느슨한 강등) |
| 장바구니 | `order_generator.py`의 장바구니 크기 분포 (1~10개) |
| 컨테이너 관리 | 실행 시 충돌 컨테이너 자동 중지/종료 후 재시작 |
