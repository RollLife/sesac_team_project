...
### 시나리오 시스템 (21가지 프리셋)
실행 중 `scenario_changer.py`로 실시간 시나리오 전환이 가능합니다:

```bash
# 시나리오 전환 (별도 터미널에서 실행)
python apps/seeders/scenario_changer.py
> 21   # 트래픽 폭증 (스트레스 테스트) 시작
> 0    # 기본 패턴으로 복귀
```

| 카테고리 | 시나리오 예시 | 특징 |
|----------|-------------|------|
| 성별/연령 특화 | 여성 대량 유입, MZ세대 트렌드 | 성별/연령 가중치 변경 |
| 시즌/이벤트 | 블랙프라이데이, 설날/추석 | 주문량 증가 + 카테고리 편중 |
| 카테고리 특화 | 뷰티 바이럴, 가전 빅세일 | 특정 카테고리 집중 |
| 트래픽 패턴 | 심야 소량, **트래픽 폭증(21번)** | 시스템 부하 테스트 |

> **21번 트래픽 폭증**: ~135 TPS (기본 대비 약 200~300배), 자동 종료 없이 수동 복귀 필요
...
| 시나리오 | 21가지 이벤트 (설날, 블프, 여름, 트래픽 폭증 등) |
